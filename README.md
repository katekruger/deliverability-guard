...
Exit codes: see `deliverability-guard --help`'s own exit-code section — that epilog (`cli.py`'s `_EXIT_CODE_EPILOG`) is this project's one canonical copy of the map, and `cli.py`'s module docstring points at the same place rather than keeping a second copy of its own. (CLOSE10-1: this line used to restate the map itself and drifted stale; CLOSE11-2: "restate it in one FEWER places" was still two places, so this is now a pointer, not a copy, per `AGENTS.md` item 5 — a test in `tests/test_cli.py` fails if this line ever grows the map's own detail back in.)
