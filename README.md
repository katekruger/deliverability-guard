...
Exit codes: `0` all clear, `1` `check` found a breach (or `resume` was refused), `2` a config/setup error (bad YAML, unknown provider, missing credential), `3` a provider transport failure (network error, rate limit exhausted, malformed response) **or a decision-log write failure** (a read-only mount, a full disk — `resume`'s own `append_resume_record` call, and `check`/`run`'s own decision-record writes, can each fail this way) — distinct from `1` so a cron wrapper can tell "the fleet is healthy" apart from "we couldn't even ask the provider" or "we couldn't record what we did." This list is a summary, not the source of truth — `cli.py`'s module docstring and `deliverability-guard --help`'s epilog are (CLOSE10-1: this line drifted out of sync with both once before; if it looks stale, trust `--help`).
...
**The "campaign only" pause marks above describe driver-API surface the breaker itself does not reach (CLOSE4-3).** `engine.breaker.evaluate`'s ladder acts strictly per-mailbox — it only ever constructs a `MailboxRef`, never a `CampaignRef` — so through the CLI today, PAUSE actually executes against exactly one provider (`instantly`), and THROTTLE against exactly one (`smartlead`); no single provider can do both. Lemlist, Apollo, and SES's campaign-/configuration-set-level `pause()` calls are real, tested driver methods (`pause_campaign`, `abort_sequence`, `pause_configuration_set`), reachable by calling them directly, but the breaker's own evaluation loop returns an honest `UNSUPPORTED` for those providers rather than ever constructing the `CampaignRef` that would let them act — deliberately: pausing an entire campaign to handle one mailbox's bad evidence is a disproportionate action this project has not decided the breaker should take automatically (see `providers/smartlead.py`'s and `providers/ses.py`'s own module docstrings for the same reasoning applied per-driver). Teaching the ladder to pause a `CampaignRef` when no per-mailbox primitive exists is real, undecided v0.2 scope, not a bug.
